...
/**
 * Classe ValidationCategoryService
 * 
 *  @author Emanoel Duarte <emanoel.c.duarte@hotmail.com>
 * 
 * Esta classe é responsável por validar os dados de um formulário de cadastro e edição de categorias, aplicando regras de validação para criação e edição de categorias.
...
class ValidationCategoryService
{
     ...
     * 
     * Este método valida os dados fornecidos no formulário de categorias, aplicando diferentes regras dependendo se é uma criação ou edição de categorias.
     * 
    ...
    public function validate(array $data): array
    {
        ...
        if (!isset($data['id'])) {
            $rules['name'] = 'required|uniqueInColumns:adms_daman_categories,name';
        } else {
            // Para edição, adicionar a validação de id e ignorar a própria categoria na verificação do nome
            $rules['id'] = 'required|integer';
            $rules['name'] = 'required|uniqueInColumns:adms_daman_categories,name,' . $data['id'];
        }
        ...
        $messages = [
            ...
            'name:uniqueInColumns'      => 'Já existe uma categoria com este nome.',
        ];
